...
def extractWords(image, thresh, lines):
...
def extractSlant(thresh, words):
	...
	if(max_index == 0):
		...
		p = s_function[3] / s_function[5]
		q = s_function[2] / s_function[6]
		print 'p='+str(p)+' q='+str(q)
		# the constants here are abritrary but I think suits the best
		if((p <= 1.1 and p >= 0.9) and (q <= 1.2 and q >= 0.8)):
			max_index = 9
			angle = 180
...
def main():
	...
	straightened = straighten(image, thresh)
	...
	lthresh = threshold(sfiltered, 160)
	#cv2.imshow('lthresh', lthresh)
	...
	lineIndices = extractLines(straightened, lthresh)
	
	#print lineIndices
	#print
	
	# convert to grayscale and binarize the image by INVERTED binary thresholding
	wthresh = threshold(sfiltered, 180)
	#cv2.imshow('wthresh', wthresh)
	...
	wordCoordinates = extractWords(straightened, wthresh, lineIndices)
